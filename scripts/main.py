...
class MarkdownParser:
    ...
    def normalize_case(self, s: str) -> str:
        """Normalize case for category names."""
                    ...
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        for line in f:
                            ...
                            for title_text, url in seebug_matches:
                                ...
                                normalized_categories = [self.normalize_case(cat) for cat in full_categories]
                            ...
                            for url in url_matches:
                                # Skip if this URL was already processed as a Seebug URL
                                if 'paper.seebug.org' in url:
                                    continue
...
class URLProcessor:
    ...
    def process_github_url(self, url):
        """Process GitHub repository URLs and user/organization URLs."""
    ...
    def process_gitlab_url(self, url):
        ...
        try:
            ...
            response = self.session.get(f'https://gitlab.com/api/v4/projects/{project_path.replace("/", "%2F")}')
    ...
    def process_crates_url(self, url):
        ...
        try:
            ...
            response = self.session.get(f'https://crates.io/api/v1/crates/{crate_name}')
    ...
    def process_gitee_url(self, url):
        ...
        try:
            ...
            response = self.session.get(f'https://gitee.com/api/v5/repos/{owner}/{repo}')
    ...
    def process_seebug_url(self, url):
        """Process Seebug URLs by extracting title from the URL path."""
        try:
            # Extract the paper ID from the URL
            # URL format: https://paper.seebug.org/1349/
            pattern = r'https://paper\.seebug\.org/(\d+)/?'
            match = re.match(pattern, url)
            ...
            if not match:
                return {
                    ...
                    'repo_pushed_at': None,
                    'status': 0,
                    'error': f"Invalid Seebug URL format: {url}"
                }
            
            paper_id = match.group(1)
            
            # For Seebug URLs, we'll use a generic title format
            # The actual title would need to be extracted from the markdown link text
            # but since we don't have access to that here, we'll use a generic format
            title = f"Seebug Paper {paper_id}"
            ...
            return {
                ...
                'repo_pushed_at': None,
                'status': 0,
                'error': f"Seebug processing error: {str(e)}"
            }
        ...
        try:
            ...
            if response.status_code != 200:
                return {
                    ...
                    'repo_pushed_at': None,
                    'status': 0,
                    'error': f"HTTP {response.status_code}: {url} ({response.text})"
                }
    ...
    def process_url(self, url, title=None):
        ...
        # If title is already provided (e.g., from Seebug markdown links), use it directly
        if title and 'paper.seebug.org' in url:
            return {
                'title': title,
                'stars': 0,
                'repo_updated_at': None,
        ...
        elif 'paper.seebug.org' in url:
            return self.process_seebug_url(url)
...
class ListsManager:
    ...
    def show_failed_entries(self):
        """Show only failed entries from the database."""
        data = self.db_manager.get_failed_entries()
        
        if not data:
            print("No failed entries found.")
            return
        
        print(f"Found {len(data)} failed entries:")
...
def main():
    ...
    if not args.update and not args.dump and not args.failed and not args.delete_failed and not args.test_url and not args.dump_minimized:
        parser.print_help()
        sys.exit(1)
